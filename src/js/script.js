...
let buttnopenmodal = document.querySelector("#openmodaladdequi")
...
buttnopenmodal.addEventListener("click" , () => {
    document.querySelector("#addequipementmodal").classList.remove("hidden")
})
closmodal.addEventListener("click" , () => {
    document.querySelector("#addequipementmodal").classList.add("hidden")
})
...
tablecoour.addEventListener("click" , (e) => {
    if(e.target.closest(".modifier")){
        ...
        fetch("src/php/recherchmodi.php" ,  {
            method : "POST" ,
...
tableEqui.addEventListener("click" , (e) => {
    if(e.target.closest(".modifier")){
        let id = e.target.closest("[data-id]").dataset.id ;
        fetch("src/php/rexhrcheequipmod.php" , {
            method : "POST",
            headers : {"Content-Type" : "application/x-www-form-urlencoded"},
            body : "id=" + id
        })
        ...
        <div id="editcoursmodal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="bg-white w-1/2 max-w-lg rounded-2xl shadow-2xl p-8 relative">
                ...
                <form id="editCoursForm">
                    ...
                    <select id="edit-categorie" name="categorie" class="w-full mb-4 p-3 rounded-lg border focus:ring-2 focus:ring-indigo-600" required>
                        <option value="" disabled selected hidden>Sélectionner une catégorie</option>
                        <option value="Yoga" ${data.categorie === 'Yoga' ? 'selected' : ''}>Yoga</option>
                        <option value="Musculation" ${data.categorie === 'Musculation' ? 'selected' : ''}>Musculation</option>
                        <option value="Cardio" ${data.categorie === 'Cardio' ? 'selected' : ''}>Cardio</option>
                        <option value="Pilates" ${data.categorie === 'Pilates' ? 'selected' : ''}>Pilates</option>
                        <option value="Crossfit" ${data.categorie === 'Crossfit' ? 'selected' : ''}>Crossfit</option>
                    </select>
...
function createmodalmodifequi(data) {

    // Remove existing modal if any
    const exist = document.querySelector("#editequipmodal");
    if (exist) exist.remove();

    const modalHTML = `
        <div id="editequipmodal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="bg-white w-1/2 max-w-lg rounded-2xl shadow-2xl p-8 relative">

                <!-- Close Button -->
                <button id="closeEditEquipModal" class="absolute top-4 right-4 text-gray-500 hover:text-red-600 text-2xl cursor-pointer">
                    &times;
                </button>

                <h2 class="text-2xl font-bold mb-6 text-indigo-700 flex items-center gap-2">
                    <i class="fas fa-tools"></i> Modifier l'équipement
                </h2>
                ...
                <form id="editEquipForm">
                    ...
                    <label class="block mb-2 font-semibold text-gray-700">Type</label>
                    <select name="type" class="w-full mb-4 p-3 rounded-lg border focus:ring-2 focus:ring-indigo-600">
                        <option value="Tapis de course" ${data.type === "Tapis de course" ? "selected" : ""}>Tapis de course</option>
                        <option value="Haltères" ${data.type === "Haltères" ? "selected" : ""}>Haltères</option>
                        <option value="Ballons" ${data.type === "Ballons" ? "selected" : ""}>Ballons</option>
                        <option value="Vélos" ${data.type === "Vélos" ? "selected" : ""}>Vélos</option>
                        <option value="Cordes" ${data.type === "Cordes" ? "selected" : ""}>Cordes</option>
                    </select>

                    <label class="block mb-2 font-semibold text-gray-700">Quantité</label>
                    <input name="quuantiter" type="number" value="${data.quuantiter}"
                        class="w-full mb-4 p-3 rounded-lg border focus:ring-2 focus:ring-indigo-600">

                    <label class="block mb-2 font-semibold text-gray-700">État</label>
                    <select name="etat" class="w-full mb-6 p-3 rounded-lg border focus:ring-2 focus:ring-indigo-600">
                        <option value="Bon" ${data.etat === "a remplacer" ? "selected" : ""}>a remplacer</option>
                        <option value="Moyen" ${data.etat === "Bon" ? "selected" : ""}>Bon</option>
                        <option value="Mauvais" ${data.etat === "moyen" ? "selected" : ""}>moyen</option>
                    </select>
                    ...
                    <div class="flex gap-4">
                        ...
                        <button type="button" id="cancelEditEquip"
                                class="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-700 py-3 rounded-xl font-semibold transition-all">
                            Annuler
                        </button>
                    </div>
    ...
    // Add events
    const modal = document.querySelector("#editequipmodal");
    const btnClose = document.querySelector("#closeEditEquipModal");
    const btnCancel = document.querySelector("#cancelEditEquip");
    const form = document.querySelector("#editEquipForm");
    ...
    form.addEventListener("submit", (e) => {
        ...
        const modifierdta = Object.fromEntries(formdata) ; 

        fetch("src/php/modifierequipement.php" , {
            method : "POST" , 
            headers : {"Content-Type" : "application/json"},
            body : JSON.stringify({  newdata : modifierdta })
        })         
            .then(rep => rep.json())
            .then(data => {
                let row = document.querySelector(`[data-id='${data.id}']`)
                if(row){
                    row.innerHTML = ``
                    row.innerHTML = `
                        <td class="px-6 py-4 text-left font-bold"> ${data.nom} </td>
                        <td class="px-6 py-4 text-left font-bold"> ${data.type} </td>
                        <td class="px-6 py-4 text-left font-bold"> ${data.quuantiter} </td>
                        <td class="px-6 py-4 text-left font-bold"> ${data.etat} </td>
                        <td class="px-6 py-4 text-left font-bold">
                            <svg xmlns="http://www.w3.org/2000/svg" data-id="${data.id}" class="modifier size-5 cursor-pointer" viewBox="0 0 26 26">
                                <path fill="#000000" d="M20.094.25a2.245 2.245 0 0 0-1.625.656l-1 1.031l6.593 6.625l1-1.03a2.319 2.319 0 0 0 0-3.282L21.75.937A2.36 2.36 0 0 0 20.094.25zm-3.75 2.594l-1.563 1.5l6.875 6.875L23.25 9.75l-6.906-6.906zM13.78 5.438L2.97 16.155a.975.975 0 0 0-.5.625L.156 24.625a.975.975 0 0 0 1.219 1.219l7.844-2.313a.975.975 0 0 0 .781-.656l10.656-10.563l-1.468-1.468L8.25 21.813l-4.406 1.28l-.938-.937l1.344-4.593L15.094 6.75L13.78 5.437zm2.375 2.406l-10.968 11l1.593.343l.219 1.47l11-10.97l-1.844-1.843z"/>
                            </svg>
                            <svg xmlns="http://www.w3.org/2000/svg" data-id="<?= $courrentrow['id'] ?>" class="supprumer size-5 cursor-pointer" viewBox="0 0 24 24">
                                <path fill="none" stroke="#000000" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="m19.5 5.5l-.62 10.025c-.158 2.561-.237 3.842-.88 4.763a4 4 0 0 1-1.2 1.128c-.957.584-2.24.584-4.806.584c-2.57 0-3.855 0-4.814-.585a4 4 0 0 1-1.2-1.13c-.642-.922-.72-2.205-.874-4.77L4.5 5.5M3 5.5h18m-4.944 0l-.683-1.408c-.453-.936-.68-1.403-1.071-1.695a2 2 0 0 0-.275-.172C13.594 2 13.074 2 12.035 2c-1.066 0-1.599 0-2.04.234a2 2 0 0 0-.278.18c-.395.303-.616.788-1.058 1.757L8.053 5.5m1.447 11v-6m5 6v-6" color="currentColor"/>
